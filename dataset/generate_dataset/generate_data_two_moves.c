...
int main() {
    ...
    for (int game = 0; game < games; ++game) {
        ...
        int last_board_state[(BOARD_DIM + 2) * (BOARD_DIM + 2) * 2] = {0};
        int prev_board_state[(BOARD_DIM + 2) * (BOARD_DIM + 2) * 2] = {0};
        ...
        while (!hg_full_board(&hg)) {
            ...
            if (hg_winner(&hg, player, hg.moves[num_states - 1][1])) {
                ...
                // Only save if at least two moves have been made
                if (num_states >= 2) {
                    if (rand() % 100 < 67) {
                        save_board_state(train_file, last_board_state, winner);
                    } else {
                        save_board_state(test_file, last_board_state, winner);
                    }
                }
                break;
            }

            // Update board states after making the move and checking for a winner
            if (num_states >= 2) {
                copy_board(prev_board_state, last_board_state);
            }
            copy_board(hg.board, prev_board_state);
